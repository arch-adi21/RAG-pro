...
load_dotenv()
api_key="AIzaSyAz9ID2ZAX-RcjPAam3Go37VdTbz2_aC2c"
...
def generate_answer(prompt):
    genai.configure(api_key="AIzaSyAz9ID2ZAX-RcjPAam3Go37VdTbz2_aC2c")
    print("GEMINI API KEY:","AIzaSyAz9ID2ZAX-RcjPAam3Go37VdTbz2_aC2c")
...
class VideoChat:
    def __init__(self):
        self.faiss_vector_store = self.process_youtube_data()
        self.embedding_function = HuggingFaceEmbeddings(model_name='sentence-transformers/all-MiniLM-L6-v2')
        self.llm = HuggingFaceHub(repo_id="google/flan-t5-large", model_kwargs={"temperature":0.5, "max_length":512})
    ...
    def chat(self, user_input):
        retriever = self.faiss_vector_store.as_retriever(search_kwargs={"k": 3})
        qa_chain = RetrievalQA.from_chain_type(self.llm, retriever=retriever)
        return qa_chain.run(user_input)

# Usage
chat_bot = VideoChat()
response = chat_bot.chat("What are the main topics discussed in these videos?")
print(response)